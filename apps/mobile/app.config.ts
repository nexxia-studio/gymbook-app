...
if (isViniz) {
  ...
  // ── 🔴 GYM-347 — CES TROIS ASSETS ÉTAIENT DÉJÀ ÉCRITS, ET PERSONNE NE LES A VUS ─────
  // Le bloc qui suit et les trois fichiers qu'il désigne viennent de la PR #264, relue et
  // fusionnée le 07/09 à 16 h 24 — mais DANS LA BRANCHE `gym-eas-production-viniz`, que la
  // PR #263 venait d'emporter dans `develop` VINGT-SEPT SECONDES PLUS TÔT. La branche n'a
  // jamais été refusionnée : GitHub affichait #264 « merged », et le travail est resté trois
  // commits en avance, invisible, pendant huit jours. Constat TestFlight : l'app « Viniz »
  // (build 2, 1.0.0) portait le logo Dopamine.
  //
  // ⚠️ LE TEXTE CI-DESSOUS EST CELUI DE #264, REPRIS TEL QUEL. Le réécrire ferait diverger
  // deux rédactions de la même décision — et c'est la rédaction relue qui doit survivre.
  // Les trois fichiers sont restaurés OCTET POUR OCTET depuis ce même commit (sha256
  // vérifiés) : aucune regénération, donc les mesures ci-dessous restent celles qui ont été
  // contrôlées. Elles ont malgré tout été REFAITES sur les pixels — 58,5 % radial, aucun
  // canal alpha, aucun chunk tRNS — et elles concordent.
